...
func (r *OrgRepository) Create(org *domain.Organization) error {
	...
	query := `INSERT INTO organizations (id, name, plan_id, status, created_at, address, phone, email, website) 
			  VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`

	_, err := r.DB.Exec(query, org.ID, org.Name, org.PlanID, org.Status, org.CreatedAt, org.Address, org.Phone, org.Email, org.Website)
	return err
}

func (r *OrgRepository) FindByID(id string) (*domain.Organization, error) {
	query := `SELECT id, name, plan_id, status, created_at, address, phone, email, website FROM organizations WHERE id = $1`
	row := r.DB.QueryRow(query, id)

	var org domain.Organization
	if err := row.Scan(&org.ID, &org.Name, &org.PlanID, &org.Status, &org.CreatedAt, &org.Address, &org.Phone, &org.Email, &org.Website); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
